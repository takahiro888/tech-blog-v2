...
- 変数・プロパティ・型のフィールド名、およびURLの`searchParams`のキー名は略語を避け、名前だけで意味が伝わるようにする（例: 検索キーワードのURLパラメータ・変数名はどちらも`q`ではなく`keyword`、パース結果は`n`ではなく`parsed`のように書く。URLパラメータの一覧は[04_ui_design.md](../../docs/global_design/04_ui_design.md)5.5節を参照）
  - このプロジェクトのURLパラメータは自分たちで設計しているものなので、他システムとの取り決めのように変更できない制約はない。わかりにくい名前に気づいたら、コード側の変数名だけでなく、`searchParams`のキー名・`<form>`の`name`属性・`04_ui_design.md`側の表記も含めて統一的にリネームする
  - 逆に、外部API・Webhook・OAuthコールバックなど、こちらで名前を決められないパラメータを扱う場合は、受け取った直後に意味の分かる変数名へ詰め替えてから内部ロジックに渡す（外部側のキー名自体は変えられないため）
